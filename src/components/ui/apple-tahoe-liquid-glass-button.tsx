...
export const LiquidGlassViewport = React.forwardRef<HTMLDivElement, LiquidGlassViewportProps>(
  ({ bgImage, fallbackMode = "webgl", className, children, ...props }, ref) => {
    ...
    React.useEffect(() => {
      ...
      const loop = () => {
        const buttons = Object.values(buttonsRef.current) as HTMLButtonElement[];
        if (buttons.length === 0) {
        ...
        // Fast key check to avoid running heavy CPU loops when elements haven't moved
        const bRects = buttons.map(btn => btn.getBoundingClientRect());
        const stateKey = `${pRect.left.toFixed(1)},${pRect.top.toFixed(1)},${pRect.width.toFixed(1)},${pRect.height.toFixed(1)}_${bRects.map(r => `${r.left.toFixed(1)},${r.top.toFixed(1)},${r.width.toFixed(1)},${r.height.toFixed(1)}`).join("|")}`;

        if (stateKey !== lastStateKey) {
          lastStateKey = stateKey;

          buttons.forEach((btn, idx) => {
            const rect = bRects[idx];
  ...
  ({ className, children, ...props }, ref) => {
    ...
    return (
      ...
      >
        ...
        <span className="relative z-20 text-sm font-semibold tracking-wide text-white select-none pointer-events-none flex items-center justify-center gap-2 drop-shadow-xs">
          {children}
        </span>
      </button>
    );
